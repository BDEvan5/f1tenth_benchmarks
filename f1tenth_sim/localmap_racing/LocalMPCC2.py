...
class LocalMPCC2:
    def __init__(self, test_id, save_data=False):
        ...
        self.mpcc_data_path = f"Logs/{self.name}/RawData_{test_id}/MPCCData_{test_id}/"
        ensure_path_exists(self.mpcc_data_path)
        self.local_map_generator = LocalMapGenerator(f"Logs/{self.name}/", test_id, save_data)
    ...
    def plan(self, obs):
        ...
        print(f"{self.counter} -- S: {100*obs['progress']:.2f} --> Pose: {obs['pose']} --> Action: {action}")

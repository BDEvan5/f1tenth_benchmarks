...
class GlobalMPCC2(BasePlanner):
    def __init__(self, test_id, save_data=False, planner_name="GlobalMPCC2"):
    ...
    def init_optimisation(self):
        states = ca.MX.sym('states', NX) # [x, y, psi, v, s]
        controls = ca.MX.sym('controls', NU) # [delta, a, p]

        rhs = ca.vertcat(states[3] * ca.cos(states[2]), 
                         states[3] * ca.sin(states[2]), 
                         (states[3] / self.vehicle_params.wheelbase) * ca.tan(controls[0]), 
                         controls[1],
    ...
    def init_constraints(self):
        ...
        lbx = [[p.position_min], [p.position_min], [-p.heading_max], [p.min_speed], [0]] * (self.N + 1) + [[-p.max_steer], [-p.max_decceleration], [p.local_path_speed_min]] * self.N
        self.lbx = np.array(lbx)
        ubx = [[p.position_max], [p.position_max], [p.heading_max], [p.max_speed], [p.local_path_max_length]] * (self.N + 1) + [[p.max_steer], [p.max_acceleration], [p.local_path_speed_max]] * self.N
    ...
    def init_bounds(self):
        ...
        for k in range(self.N):
            ...
            force_lateral = self.X[3, k] **2 / self.vehicle_params.wheelbase * ca.tan(ca.fabs(self.U[0, k])) *  self.vehicle_params.vehicle_mass
    ...
    def init_bound_limits(self):
        ...
        for k in range(self.N):  # set the reference controls and path boundary conditions to track
            self.lbg[NX - 1 + (NX + 2) * (k + 1), 0] = - self.f_max
            self.ubg[NX - 1 + (NX + 2) * (k + 1) , 0] = self.f_max
    ...
    def prepare_input(self, obs):
        x0 = np.append(obs["pose"], obs["vehicle_speed"])
        x0 = np.append(x0, self.centre_line.calculate_progress_m(obs["pose"][0:2]))
    ...
    def plan(self, obs):
        ...
        action[1] = obs['vehicle_speed'] + action[1] * self.dt
    ...
    def construct_warm_start_soln(self, initial_state):
        ...
        for k in range(1, self.N + 1):
            ...
            next_point = self.centre_interpolant.get_point(s_next) 
            ...
            if self.X0[k-1, 3] < self.planner_params.p_initial:
                next_speed = self.X0[k-1, 3] + self.planner_params.max_acceleration * self.dt
            else:
                next_speed = self.X0[k-1, 3]
            self.X0[k, :] = np.array([next_point[0], next_point[1], psi_next, next_speed, s_next])
            # self.X0[k, :] = np.array([x_next.full()[0, 0], y_next.full()[0, 0], psi_next, self.planner_params.p_initial, s_next])
